...
#define SHAKTI_UART_BAUD         0
#define SHAKTI_UART_TX_REG       1
#define SHAKTI_UART_RCV_REG      2
#define SHAKTI_UART_STATUS       3
#define SHAKTI_UART_DELAY        4
#define SHAKTI_UART_CONTROL      5
#define SHAKTI_UART_IEN          6
#define SHAKTI_UART_RX_THRESHOLD 8
...
static uintptr_t uart_shakti_reg(const struct uart_shakti_config *cfg, uint8_t reg)
{
	return cfg->base + ((uintptr_t)reg << cfg->reg_shift);
}

static int uart_shakti_poll_in(const struct device *dev, unsigned char *c)
{
	const struct uart_shakti_config *cfg = dev->config;
	uint32_t status = sys_read32(uart_shakti_reg(cfg, SHAKTI_UART_STATUS));

	if ((status & STATUS_RX_NOT_EMPTY) == 0U) {
		return -1;
	}

	*c = (unsigned char)(sys_read32(uart_shakti_reg(cfg, SHAKTI_UART_RCV_REG)) & 0xffU);
	return 0;
}

/*
 * V3 TX trigger: after writing TX_REG, write baud, delay, IEN,
 * and rx_threshold to start transmission. Uses cached values
 * to avoid slow MMIO reads.
 */
static void uart_shakti_tx_trigger(const struct device *dev)
{
	...
	sys_write32(cfg->baud_divisor, uart_shakti_reg(cfg, SHAKTI_UART_BAUD));
	sys_write32(0U, uart_shakti_reg(cfg, SHAKTI_UART_DELAY));
	sys_write32(data->ien_shadow, uart_shakti_reg(cfg, SHAKTI_UART_IEN));
	sys_write32(0U, uart_shakti_reg(cfg, SHAKTI_UART_RX_THRESHOLD));
}
...
static void uart_shakti_poll_out(const struct device *dev, unsigned char c)
{
	...
	while ((sys_read32(uart_shakti_reg(cfg, SHAKTI_UART_STATUS)) & STATUS_TX_FULL) != 0U) {
		;
	}

	/* V3 TX sequence: write data, then trigger */
	sys_write32((uint32_t)c, uart_shakti_reg(cfg, SHAKTI_UART_TX_REG));
...
static int uart_shakti_fifo_fill(const struct device *dev, const uint8_t *tx_data, int size)
{
	...
	for (i = 0; i < size; i++) {
		if ((sys_read32(uart_shakti_reg(cfg, SHAKTI_UART_STATUS)) & STATUS_TX_FULL) != 0U) {
			break;
		}
		sys_write32((uint32_t)tx_data[i], uart_shakti_reg(cfg, SHAKTI_UART_TX_REG));
		uart_shakti_tx_trigger(dev);
	}
...
static int uart_shakti_fifo_read(const struct device *dev, uint8_t *rx_data, const int size)
{
	...
	for (i = 0; i < size; i++) {
		if ((sys_read32(uart_shakti_reg(cfg, SHAKTI_UART_STATUS)) & STATUS_RX_NOT_EMPTY) ==
		    0U) {
			break;
		}
		rx_data[i] =
			(uint8_t)(sys_read32(uart_shakti_reg(cfg, SHAKTI_UART_RCV_REG)) & 0xffU);
	}
...
static void uart_shakti_irq_tx_enable(const struct device *dev)
{
	...
	data->ien_shadow |= (IEN_TX_FULL | IEN_TX_EMPTY);
	sys_write32(data->ien_shadow, uart_shakti_reg(cfg, SHAKTI_UART_IEN));
}
...
static void uart_shakti_irq_callback_set(const struct device *dev, uart_irq_callback_user_data_t cb,
					 void *cb_data)
{
	struct uart_shakti_data *data = dev->data;
...
static int uart_shakti_init(const struct device *dev)
{
	...
	sys_write32(8U << 5, uart_shakti_reg(cfg, SHAKTI_UART_CONTROL));

	/* Clear delay, interrupt enable, and RX threshold */
	sys_write32(0U, uart_shakti_reg(cfg, SHAKTI_UART_DELAY));
	sys_write32(0U, uart_shakti_reg(cfg, SHAKTI_UART_IEN));
	sys_write32(0U, uart_shakti_reg(cfg, SHAKTI_UART_RX_THRESHOLD));
...
#define UART_SHAKTI_IRQ_CFG_FUNC(n)                                                                \
	...
	{                                                                                          \
		IRQ_CONNECT(DT_INST_IRQN(n), DT_INST_IRQ(n, priority), uart_shakti_irq_handler,    \
...
#define UART_SHAKTI_INIT(n)                                                                        \
	...
	static struct uart_shakti_data uart_shakti_data_##n;                                       \
	static const struct uart_shakti_config uart_shakti_cfg_##n = {                             \
		...
		.reg_shift = DT_INST_PROP(n, reg_shift),                                           \
		UART_SHAKTI_IRQ_CFG_FUNC_INIT(n)};                                                 \
	DEVICE_DT_INST_DEFINE(n, uart_shakti_init, NULL, &uart_shakti_data_##n,                    \
			      &uart_shakti_cfg_##n, PRE_KERNEL_1, CONFIG_SERIAL_INIT_PRIORITY,     \
