...
| Effective throughput | 21 GMAC/s = 42 GOPS |
...
```bash
bash /home/afonso/zephyr/local-artifacts/erbium_amp_probe/luxonis-real-dncnn/build_tfma_int8_v2.sh
...
```bash
TIMEOUT=180 /home/afonso/zephyr/local-artifacts/erbium_amp_probe/luxonis-real-dncnn/run_int8_tfma_v2.sh
...
ref = np.fromfile('/home/afonso/zephyr/local-artifacts/erbium_amp_probe/luxonis-real-dncnn/dncnn3_luxonis_240x320_ort_output_f32.bin', dtype=np.float32).reshape(240,320)
...
```bash
python3 /home/afonso/zephyr/local-artifacts/erbium_amp_probe/luxonis-real-dncnn/tools/build_viz_html.py
...
- `run_int8_tfma_v2.sh` — stage + run on `esperanto-soc4`
- `audit_master_tfma_fp32.tsv` (in `etsoc1-luxonis-experiments/results/`)
