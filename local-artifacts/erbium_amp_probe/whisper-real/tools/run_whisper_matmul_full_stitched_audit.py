#!/usr/bin/env python3
"""Run a full stitched silicon audit for one exported Whisper MatMul node."""
...
def manifest_label(manifest: dict[str, Any], *keys: str, default: str = "") -> str:
    for key in keys:
...
def layout_for(max_tile_rows: int, k_dim: int, n_cols: int) -> dict[str, int]:
    out_offset = 0x4000
    out_len = max_tile_rows * n_cols * 4
    act_offset = align_up(out_offset + out_len)
    act_len = max_tile_rows * k_dim * 4
    wt_offset = align_up(act_offset + act_len)
    wt_len = n_cols * k_dim * 4
    ref_offset = align_up(wt_offset + wt_len)
    ref_len = max_tile_rows * n_cols * 4
...
def build_tile(tile_id: int, row0: int, row1: int, build_root: Path,
    ...
    act_full = np.fromfile(data_dir / manifest["files"]["act_full"],
                           dtype=np.float32).reshape(int(manifest["rows"]), k_dim)
    ref_full = np.fromfile(data_dir / manifest["files"]["ref_full"],
                           dtype=np.float32).reshape(int(manifest["rows"]), n_cols)
    weight_t = np.fromfile(data_dir / manifest["files"]["weight_t"],
                           dtype=np.float32).reshape(n_cols, k_dim)

    # The C file still uses the original MLP0 binary-symbol names. Keep these
    # filenames stable and pass the actual dimensions through -D defines.
    ...
    elf = build_root / f"matmul_t{tile_id:02d}_{row0}_{row1}.elf"
    cmd = [
        GCC,
        *flags,
        *BASE_FLAGS,
...
def remote_script(tiles: list[tuple[int, int, int]], remote_work: str,
                  timeout: int, n_cols: int, out_offset: int,
                  active_harts: int) -> str:
...
"""
    for tile_id, row0, row1 in tiles:
        rows = row1 - row0
        elf = f"matmul_t{tile_id:02d}_{row0}_{row1}.elf"
        script += f"""
...
def main() -> int:
    ...
    mem = layout_for(args.tile_rows, k_dim, n_cols)

    tiles = []
    for row0 in range(0, rows_total, args.tile_rows):
        row1 = min(row0 + args.tile_rows, rows_total)
        tiles.append((len(tiles), row0, row1))

    stamp = time.strftime("%Y%m%d-%H%M%S")
    build_root = ROOT / "phase0" / "matmul_full_builds" / f"{args.variant}_{stamp}"
    ...
    elfs = [
        build_tile(tile_id, row0, row1, build_root, flags, args.data_dir,
                   manifest, mem, args.active_harts)
        for tile_id, row0, row1 in tiles
    ]
    ...
    subprocess.run(
        [*SSH_CMD, "bash", "-s"],
        input=remote_script(tiles, remote_work, args.timeout, n_cols,
                            mem["out_offset"], args.active_harts),
        text=True,
        check=True,
    )
    ...
    ref_full = np.fromfile(args.data_dir / manifest["files"]["ref_full"],
                           dtype=np.float32).reshape(rows_total, n_cols)
    stitched = np.zeros((rows_total, n_cols), dtype=np.float32)
    tile_reports = []
    total_wait = 0.0
    tile_ok = True
    for tile_id, row0, row1 in tiles:
    ...
    ops = rows_total * k_dim * n_cols * 2
    full = {
        "variant": args.variant,
        "timestamp": stamp,
        "onnx_node": manifest_label(manifest, "node_name", "out_name", "output_name"),
        "act_name": manifest_label(manifest, "act_name", "input_name"),
        "weight_name": manifest_label(manifest, "weight_name", default="conv_weight"),
        "rows": rows_total,
        "k_dim": k_dim,
        "n_cols": n_cols,
        "tiles": len(tiles),
    ...
    with tsv.open("a", newline="") as f:
        cols = [
            "timestamp", "variant", "onnx_node", "act_name", "weight_name",
            "rows", "k_dim", "n_cols", "tiles", "tile_rows", "active_harts",
            "total_wait_s", "ops", "gops", "max_abs", "mean_abs", "p99_abs",
            "allclose_1e4", "allclose_1e3", "tile_ok", "remote_work",
            "local_run_dir",
        ]
