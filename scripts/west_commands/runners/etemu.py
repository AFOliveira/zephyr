...
def _elf_entry(elf_path: str) -> int:
    if ELFTOOLS_MISSING:
        raise RuntimeError('pyelftools is missing; pass --sp-reset-pc/--reset-pc or set --bootrom')
...
class EtEmuBinaryRunner(ZephyrBinaryRunner):
    ...
    def do_add_parser(cls, parser: argparse.ArgumentParser):
        parser.add_argument(
            '--sys-emu', help='path to sys_emu binary (defaults to $SYS_EMU or sys_emu in PATH)'
        )
        parser.add_argument('--bootrom', help='path to BootromTrampolineToBL2.elf (optional)')
        parser.add_argument(
            '--no-bootrom',
            action='store_true',
        ...
        parser.add_argument(
            '--reset-pc',
            type=_int_arg,
            help='override main reset PC (hex ok; required if no bootrom and pyelftools missing)',
        )
        parser.add_argument(
            '--reset-target',
            choices=('sp', 'main'),
        ...
        parser.add_argument('--shires', help='shires mask (default: emulator default)')
        parser.add_argument(
            '--max-cycles',
            type=int,
            default=20000000000,
        ...
        parser.add_argument(
            '--enable-minions', action='store_true', help='do not pass -mins_dis (enable minions)'
        )
        parser.add_argument(
            '--uart0-tx-file', help='path for SPIO UART0 TX output (default: stdout)'
        )
        parser.add_argument(
            '--uart1-tx-file', help='path for SPIO UART1 TX output (default: stdout)'
        )
        parser.add_argument('--uart0-rx-file', help='path for SPIO UART0 RX input (default: stdin)')
        parser.add_argument('--uart1-rx-file', help='path for SPIO UART1 RX input (default: stdin)')
        parser.add_argument(
            '--sys-emu-arg',
            action='append',
            metavar='ARG',
